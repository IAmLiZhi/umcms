...
public class SearchAction extends com.uoumei.mdiy.action.BaseAction{
	...
	public String index(HttpServletResponse response,HttpServletRequest request,ModelMap model){
		model.addAttribute("searchType",BasicUtil.resToMap("com.uoumei.mdiy.resources.search_type"));
	...
	public void list(@ModelAttribute SearchEntity search,HttpServletResponse response, HttpServletRequest request,ModelMap model) {
		...
		search.setAppId(managerSession.getBasicId());
		BasicUtil.startPage();
		List searchList = searchBiz.query(search);
		this.outJson(response, com.uoumei.base.util.JSONArray.toJSONString(new EUListBean(searchList,(int)BasicUtil.endPage(searchList).getTotal()),new DoubleValueFilter(),new DateValueFilter()));
	}
	...
	public String generateSreachFormHtml(ModelMap model, HttpServletRequest request) {
		...
		int cmId = 0;
		Map<String, String> basicField = getMapByProperties(com.uoumei.mdiy.constant.Const.BASIC_FIELD);
		Map<String, String> basicAttribute = getMapByProperties(com.uoumei.mdiy.constant.Const.BASIC_ATTRIBUTE);
	...
	public String searchCode(@PathVariable int searchId,ModelMap model ,HttpServletRequest request){
		// 获取登录的session --zzq
		ManagerSessionEntity managerSession = (ManagerSessionEntity) getManagerBySession(request);
		List<ColumnEntity> columnList = columnBiz.queryColumnListByWebsiteId(managerSession.getBasicId());
