...
import com.uoumei.cms.parser.impl.ArticleAuthorParser;
import com.uoumei.cms.parser.impl.ArticleContentParser;
import com.uoumei.cms.parser.impl.ArticleDateParser;
import com.uoumei.cms.parser.impl.ArticleDescripParser;
import com.uoumei.cms.parser.impl.ArticleHistoryParser;
import com.uoumei.cms.parser.impl.ArticleIdParser;
import com.uoumei.cms.parser.impl.ArticleKeywordParser;
import com.uoumei.cms.parser.impl.ArticleLinkParser;
import com.uoumei.cms.parser.impl.ArticleLitpicParser;
import com.uoumei.cms.parser.impl.ArticleSourceParser;
import com.uoumei.cms.parser.impl.ArticleTitleParser;
import com.uoumei.cms.parser.impl.ArticleTypeIdParser;
import com.uoumei.cms.parser.impl.ArticleTypeLinkParser;
import com.uoumei.cms.parser.impl.ArticleTypeTitleParser;
import com.uoumei.cms.parser.impl.ChannelParser;
import com.uoumei.cms.parser.impl.ColumnParser;
import com.uoumei.cms.parser.impl.HitParser;
...
import com.uoumei.mdiy.parser.ListParser;
import com.uoumei.mdiy.parser.TaglibParser;
import com.uoumei.parser.IParserRegexConstant;
import com.uoumei.parser.PageUtilHtml;
import com.uoumei.parser.impl.general.ChannelContParser;
import com.uoumei.parser.impl.general.PageNumParser;
import com.uoumei.parser.impl.general.PageParser;
...
//			}
			...
			ArticleEntity articleWhere = new ArticleEntity();
			articleWhere.setArticleState(com.uoumei.cms.constant.Const.ARTICLE_STATE_PUB);
			int articleCount = articleBiz.getCountByColumnId(app.getAppId(), columnIds, flags, noFlags,articleWhere);
...
//				BasicUtil.startPage(0,size,false);
				BasicUtil.startPageHtml(0,size,false);
				List<ArticleEntity> listArticles = articleBiz.query(app.getAppId(), columnIds, flags, noFlags,orderBy, order.equals("desc") ? true : false,articleWhere);
//				BasicUtil.endPage(listArticles);
				BasicUtil.endPageHtml(listArticles);
				// 替换列表标签
				htmlContent = new com.uoumei.cms.parser.impl.ListParser(app,htmlContent, listArticles,  this.getWebsiteUrl(), property, false, fieldBiz, contentBiz).parse();
			}else{
				htmlContent = new com.uoumei.cms.parser.impl.ListParser(app,htmlContent, null,  this.getWebsiteUrl(), property, false, fieldBiz, contentBiz).parse();
			}
		}
		return htmlContent;
	}
	...
	private String parseChannel() {
		...
		for (int i = 0; i < channelConNum; i++) {
			...
			if (channelTypeId != 0) {
				ArticleEntity articleWhere = new ArticleEntity();
				articleWhere.setArticleState(com.uoumei.cms.constant.Const.ARTICLE_STATE_PUB); 
				List<ArticleEntity> arctileList = articleBiz.queryListByColumnId(channelTypeId,articleWhere);
				if (arctileList != null) {
					if (arctileList.size() > 0) {
						channelCont = arctileList.get(arctileList.size() - 1).getArticleContent();
					} else {
						channelCont = arctileList.get(arctileList.size()).getArticleContent();
					}
				}
			}
			// 替换封面标签
...
//				}
				...
				if (articleCount != 0) {
					...
					BasicUtil.startPageHtml(page.getPageNo()+1,page.getPageSize(),false);
					// 从数据库取出文章列表数组
					List<ArticleEntity> listArticles = articleBiz.query(this.app.getAppId(), columnIds, flags, noFlags,  orderBy, order.equals("desc") ? true : false,articleWhere);
					// 替换列表标签
					htmlContent = new com.uoumei.cms.parser.impl.ListParser(app,htmlContent, listArticles,  this.getWebsiteUrl(), property, true, fieldBiz, contentBiz).parse();
				}else{
					htmlContent = new com.uoumei.cms.parser.impl.ListParser(app,htmlContent, null,  this.getWebsiteUrl(), property, true, fieldBiz, contentBiz).parse();
				}
			}
		}
		return htmlContent;
