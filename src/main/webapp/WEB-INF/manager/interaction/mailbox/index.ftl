<@ms.html5>
	...
	<@ms.searchForm name="searchForm" action="">
		...
		<@ms.text label="信封内容"  name="publishContent" value="" title="请输入信封内容"  placeholder="请输入信封内容" />
		<@ms.searchFormButton>
...
$(function(){
	...
	$('.sure').click(function(){
		...
		$.ajax({
			...
			success: function(data){
				if(data){
					$('.mask').css('display', 'none');
					var ss = "${managerPath}/hd/tzxx/index.do";
					location.href= ss;
				}
			},
			error: function(e){
	...
	$('#toolbars').find('select').change(function () {
		$("#columnList").bootstrapTable('destroy').bootstrapTable({
			url:"${managerPath}/hd/4/list.do"+"?stateStr="+state,
            ...
            exportOptions: {
                fileName: '厅长信箱' + new Date().toLocaleDateString(),//下载文件名称
            },
			...
			columns: [{ checkbox: true, visible: $(this).val() === 'selected'},
				{
			        ...
			        formatter: function (value, row, index){
			        	var url="${managerPath}/hd/"+row.id+"/mailbox/show.do?1";
	...
	//跳转到详情页面
	function show(id){
		var ss = "${managerPath}/hd/"+id+"/mailbox/show.do";
		location.href= ss;
	}
