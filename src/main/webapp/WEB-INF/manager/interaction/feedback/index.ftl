<@ms.html5>
	...
	<@ms.searchForm name="searchForm" action="">
		...
		<@ms.text label="反馈内容"  name="publishContent" value="" title="请输入反馈内容"  placeholder="请输入反馈内容" />
		<@ms.searchFormButton>
	...
	<div id="toolbar">
		<@ms.panelNavBtnGroup>
			...
			<div id="toolbars" class="select" style="display: inline-block; margin-left: 10px;">
	...
	<div class="mask" style="position: fixed; display: none; z-index:1001; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5);">
		<div class="alert" style="background-color: #Ffffff; width: 569px; height: 427px; margin: 70px auto 0;">
			...
			<p style="overflow: hidden;"><span class="prompt" style="display: none; color: #ff5959;">请输入内容</span><span style="float: right">最大字数限制140字。</span></p>
...
$(function(){
	...
	$('textarea').blur(function(){
		...
		if(length > 140){
			$('.prompt').css('display','').text('最多可以输入140字，现在有' + length + '字');
		}else if(length == 0){
	...
	$('.sure').click(function(){
		...
		$.ajax({
			...
			success: function(data){
				if(data){
					$('.mask').css('display', 'none');
					var ss = "${managerPath}/hd/yjfk/index.do";
					location.href= ss;
				}
			},
			error: function(e){
	...
	$('#toolbars').find('select').change(function () {
		$("#columnList").bootstrapTable('destroy').bootstrapTable({
			url:"${managerPath}/hd/3/list.do"+"?stateStr="+state,
		    ...
		    exportOptions: {
		        fileName: '意见反馈' + new Date().toLocaleDateString(),//下载文件名称
		    },
			...
			columns: [
				{ 
					...
					formatter: function (value, row, index){
						var url="${managerPath}/hd/"+row.id+"/feedback/show.do?1";
	...
	//跳转到详情页面
	function show(id){
		var ss = "${managerPath}/hd/"+id+"/feedback/show.do";
		location.href= ss;
	}
})
</script>