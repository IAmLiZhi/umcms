<@ms.html5>
	...
	<@ms.searchForm name="searchForm" action="">
		...
		<@ms.text label="咨询内容"  name="publishContent" value="" title="请输入咨询内容"  placeholder="请输入咨询内容" />
		<@ms.searchFormButton>
	...
	$(function(){
		...
		$('#toolbars').find('select').change(function () {
			$("#columnList").bootstrapTable('destroy').bootstrapTable({
				url:"${managerPath}/hd/1/list.do"+"?stateStr="+state,
	            ...
	            exportOptions: {
	                fileName: '政策咨询' + new Date().toLocaleDateString(),//下载文件名称
	            },
		    	...
		    	columns: [{ checkbox: true, visible: $(this).val() === 'selected'},
		    		{
				        ...
				        formatter: function (value, row, index){
				        	var url="${managerPath}/hd/"+row.id+"/consult/show.do?1";
    	...
    	//跳转到详情页面
    	function show(id){
    		var ss = "${managerPath}/hd/"+id+"/consult/show.do";
	     	location.href= ss;
    	}
})
