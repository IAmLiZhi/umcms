...
<@ms.content>
	...
	<@ms.contentBody width="85%" style="overflow-y: hidden;">
		...
		<iframe src="${managerPath}/cms/article/0/mymain.do" style="width:100%;maring:0;padding:0;border:none;height:100%;background-image: url(${skin_manager_loadding});  background-repeat: no-repeat;  background-position: center;" id="listFrame" target="listFrame" ></iframe>
		</@ms.contentPanel>
	</@ms.contentBody>
</@ms.content>
<script>
...
function getZtreeId(event,treeId,treeNode){
	if (treeNode.columnType==1) {
		...
		$("#listFrame").attr("src","${managerPath}/cms/article/"+treeNode.categoryId+"/mymain.do?isParent="+isParent+"&categoryTitle="+encodeURIComponent(treeNode.categoryTitle));
	} else if(treeNode.columnType==2){
		//判断该单篇栏目是否存在文章
		$.ajax({ 
			type: "POST", 
			url: base+"${baseManager}/cms/article/"+treeNode.categoryId+"/myqueryColumnArticle.do",
			dataType:"json",
		...
		$("#listFrame").attr("src","${managerPath}/cms/article/0/mymain.do"); 
	}
