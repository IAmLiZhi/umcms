...
$(function(){
	...
	$.post('${managerPath}/cms/stats/pv/list.do', {
		flag: 3,
	...
	$.post('${managerPath}/cms/stats/source/list.do', {type: 'link'}, function(data){
		for(var i = 0; i < data.length; i++){
			var $li = $('<li><p class="one">' + data[i].statisticColumnValue + '</p><p class="two">' + data[i].pv + '</p></li>');
			$('.from').append($li);
		}
	}, 'json');
	$.post('${managerPath}/cms/stats/source/list.do', {type: 'source'}, function(data){
		var option2 = {
			title : {
				text: '来源分布图',
				x:'center'
	...
	$.post('${managerPath}/cms/stats/article/list.do', {}, function(data){
		for(var i = 0; i < data.length; i++){
			var $li = $('<li><p class="one">' + data[i].basicTitle + '</p><p class="two">' + data[i].basicHit + '</p></li>')
			$('.article').append($li);
		}
	}, 'json');
	$.post('${managerPath}/cms/stats/pv/count.do', {}, function(data){
	...
	$.post('${managerPath}/cms/stats/article/count.do', {}, function(data){
