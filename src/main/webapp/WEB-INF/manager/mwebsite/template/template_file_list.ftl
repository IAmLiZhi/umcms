<@ms.html5>
	...
	<@ms.panel>
			<@shiro.hasPermission name="mwebsite:template:upload"><@ms.myuploadFile uploadFloderPath="${uploadFileUrl}" path="" inputName="file" size="20" filetype="*.htm;*.html;*.jpg;*.gif;*.png;*.css;*.js;*.ico;*.swf;*.less" msg="建议上传5M以下htm/html/css/js/jpg/gif/png/swf文件"  maxSize="5" callBack="refresh" isRename="false"/></@shiro.hasPermission> 
	           		...
	           		<#list fileNameList as fileName>
			        	<tr> 
				            ...
				            <td style="width:10%;text-align:center">	
				            	<@shiro.hasPermission name="mwebsite:template:del">	        
		                    	...
		                    	<@shiro.hasPermission name="mwebsite:template:update">	        
	...
	<script type="text/javascript">
       ...
       function chat() {  
       	  	  ...
       	  	  if($("#file").val().substring($("#file").val().lastIndexOf(".")+1) == "zip"){
	       	  	  $.ajax({
					   type: "post",
					   url: base+"${baseManager}/mwebsite/template/unZip.do",
					   data: "fileUrl=" + fileUrl,
					   success: function(msg){ 
					     	location.href=base+"${baseManager}/mwebsite/template/showChildFileAndFolder.do?websiteId="+websiteId+"&skinFolderName="+temp;
        ...
        //子站改变
	   	$("#websites").change(function(){
	   		websiteId = $("#websites").val();
	   		location.href=base+"${baseManager}/mwebsite/template/queryTemplateSkin.do?websiteId="+websiteId;
	   	})
	   	
		//删除模版文件
		$("#deleteButton").click(function(){
			
			var URL = base+"${baseManager}/mwebsite/template/deleteTemplateFile.do?websiteId="+websiteId;
		...
		$(function () {	
	       ...
	       $(".name").each(function(i){
				...
				if(suffix == "jpg" || suffix == "gif" || suffix == "png" || suffix == "jpeg" || suffix == "ico"){
			...
			$("table").delegate(".editFileBtn","click",function(){
		        ...
		        if(fileName.indexOf(".") >= 0){
		        	location.href = base+"${baseManager}/mwebsite/template/readFileContent.do?fileName="+encodeURIComponent(fileName);
		        } else {
					location.href=base+"${baseManager}/mwebsite/template/showChildFileAndFolder.do?websiteId="+websiteId+"&skinFolderName="+encodeURIComponent(fileName);
				}
			});
