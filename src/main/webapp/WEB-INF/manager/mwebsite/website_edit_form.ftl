<@ms.html5>
	...
	<@ms.panel>
			<@ms.form isvalidation=true name="websiteForm" action="${base}${baseManager}/website/${autoCURD}.do" redirect="${base}${baseManager}/website/index.do">
				...
				<@ms.select name="websiteStyle" width="300" id="websiteStyle" label="选择模板"  />
...
<script>
	$(function(){
		<#if website.websiteId != 0>
				ms.get("${managerPath}/mwebsite/template/queryAppTemplateSkin.do?websiteId=${website.websiteId}",null,function(msg){
						if(msg.fileName != null && msg.fileName.length != 0){
							//清空默认信息
							$("#websiteStyle").html("");
				   			for(var i=0; i<msg.fileName.length; i++){
				   				if ("${website.websiteStyle?default('')}"==msg.fileName[i]) {
				   					$("#websiteStyle").append("<option value="+msg.fileName[i]+" selected>"+msg.fileName[i]+"</option>")
				   				} else {
					   				$("#websiteStyle").append("<option value="+msg.fileName[i]+">"+msg.fileName[i]+"</option>")
					   			}
					   		}
				   		}else{
