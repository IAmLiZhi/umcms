<@ms.html5>
    ...
    <@ms.modalBody>
		<@ms.form name="managerForm" isvalidation=true action="${managerPath}/basic/manager/save.do" redirect="${managerPath}/basic/manager/index.do">
			...
			<@ms.text label="管理员帐号" name="managerName" value=""  width="240px;" placeholder="请输入管理员名" validation={"required":"true","minlength":"3","maxlength":"12","data-bv-stringlength-message":"管理员用户名长度为3~12个字符!", "data-bv-notempty-message":"必填项目"}/>
...
<script>
	$(function(){
		//加载选择角色列表
		$("#managerRoleID").request({url:"${managerPath}/basic/role/again/list.do",type:"json",method:"post",func:function(msg) {
			var managerArr = msg;
