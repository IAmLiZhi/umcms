$(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(var i = 0; i < list.length; i++){
                    ...
                    var $dd = $('<dd><p class="title"><img src="../../static/image/connection/exclamation-mark.png"/><a target="_top" href="javascript:;" id="' + list[i].id + '">' + list[i].title + '</a></p><p class="time">' + date + '</p><p class="already">已回复</p></dd>');
                    $dl.append($dd);
                }
                $('.limit')
                    .children('p.allNum')
    ...
    $('p.button button').click(function(){
        window.sessionStorage.setItem('key', 'jdjb');
        $(this).parent().attr({
            'href': '../361/index.html'
        })
    });
