            ...
            $oCtrl.on('click', 'span', function (event) {
                if ($(this).attr('class') == "arrow prev") {
                    ...
                    if ($over) {
                        var $that = $(this).attr('data-value');
                        if ($that > $index) {
                            $moveWrapChild.eq($index).stop().css('display', 'none').animate($moveArgs1, $playSpeed)
                        } else if ($that < $index) {
                            $moveWrapChild.eq($index).stop().css('display', 'none').animate($moveArgs1, $playSpeed)
                        }
                        $index = $that;
                        $moveWrapChild.eq($index).stop().css('display', 'block').animate($moveArgs2, $playSpeed, function () {
                            overFunc(true)
                        });
                        if ($isLibs) {
                            libsChange($index)
