...
(function ($) {
	...
	$.fn.myPostForm = function (form, config) {
		var target = $(this);
		if (isEmpty($(form).attr("action")) && isEmpty(config.action)) {
			alert("配置错误：from表单不存在action属性");
			return;
		...
		var passWord = $(form).find('#managerPassword');
		var name = $(form).find('#managerName');
		var rand_code = $(form).find('#rand_code');
		var formdate = 'managerName=' + name.val() + '&managerPassword=' + md5(passWord.val()) + '&rand_code=' + rand_code.val();
		$.ajax({
			type: "POST",
			url: action,
			dataType: data_type,
